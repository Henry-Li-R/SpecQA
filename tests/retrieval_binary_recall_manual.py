...
DEFAULT_QUERIES_PATH = REPO_ROOT / "tests/retrieval_binary_recall_manual.jsonl"
...
def run_eval(queries_path: Optional[Path] = None) -> Dict:
    ...
    for item in queries:
        total += 1
        qid = item.get("qid", f"m{total:02d}")
...
def main() -> None:
    ...
    print(f"Collection:  {COLLECTION} (manual queries, rerank={RERANK_ENABLED})")
